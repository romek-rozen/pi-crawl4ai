...
Never use Trafilatura, BM25 filtering, deep crawl, or `question`. Read and validate the saved JSON. Do not run without an extraction strategy or guess missing data. Use cache unless freshness requires `bypass_cache: true`; increase `timeout` for slow pages.
